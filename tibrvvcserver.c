 ...
 * are used.  For information on default values for the transport
 * parameters, please see the TIBCO Rendezvous Concepts manual.
 *
 * The user may specify the number of threads, and a status display
 * frequency value.  The following defaults are used.
 *   threads        1       a single dispatcher thread
 ...
 *     tibrvvcserver -service 7522: -daemon localhost:7500 -status 1000
 *
 */
...
#define SERVER_TIMEOUT      300.              /* Server times out after
                ...
                void*      closure)
{
    ...
    if (new_msg)
    {
        ...
        {
            fprintf(stderr,
                    "%s failed to send reply %d to client request: %s\n",
    ...
    /* Frequency report is based on the number of messages received by this
     * individual virtual circuit. */
    if (status_frq > 0) {
        if ((vc->msgs_in/status_frq)*status_frq == vc->msgs_in)
        {
            tibrvMsg_GetCurrentTimeString(localTime, gmtTime);
            fprintf(stderr,"%s: %d messages received, %d messages sent\n",
                    gmtTime, vc->msgs_in, vc->msgs_out);
        }
    }
}
...
destroyVC(vcRec*    vc)
{
    ...
    lost data. */
    tibrvQueueGroup_Remove(vc->server->vc_queues,
        vc->vc_queue);
    tibrvQueue_GetCount(vc->vc_queue, &queue_count);
    ...
    tibrvQueue_Destroy(vc->vc_queue);
                ...
                void*       closure)
{
    ...
    /* In case we are multithreaded and not all of the callbacks have returned,
       wait very briefly if responses out doesn't match requests in. */
    if (vc->msgs_in != vc->msgs_out)
            tibrvQueue_TimedDispatch(vc->vc_queue, 0.0005);
    ...
    tibrvEvent_Destroy(vc->vc_inbox_event);
               ...
               void*      closure)
{
    ...
    /* Add connect subject as reply inbox in the message and send it. */
     ...
     char** argv)
{
    ...
    char                disp_name[512]; /* string buffer */
    const char*         tibrv_version;

    unsigned int        i, vcthreads=0;
    ...
    else if (vcthreads < 1) vcthreads = 1; /* need at least one */
    ...
    fprintf(stderr, "Listening for client requests on subject %s.\n"
            ...
            SEARCH_SUBJECT, SERVER_TIMEOUT, gmtTime, program_name);
