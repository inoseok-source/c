    ...
    void*               closure)
{
    ...
    tibrv_u64           seqno_last_sent;
    tibrv_u64           total_msgs;
    tibrv_u64           total_size;

    tibrvMsgField       listener;
    const char*         listener_cmname    = NULL;
    tibrv_u64           last_confirmed;
    ...
    err   = tibrvMsg_GetString(message, "subject", &ldgr_subject) ||
            tibrvMsg_GetU64(message, "seqno_last_sent", &seqno_last_sent) ||
            tibrvMsg_GetU64(message, "total_msgs", &total_msgs) ||
            tibrvMsg_GetU64(message, "total_size", &total_size);
    if (err != TIBRV_OK)
    {
    ...
    printf("\nSubj: %-45s Last seqno sent: %10llu\n", ldgr_subject, seqno_last_sent);
    ...
    for (i=4;i<numFields;i++)
    {
        err   = tibrvMsg_GetFieldByIndex(message, &listener, i) ||
                ...
                tibrvMsg_GetU64(listener.data.msg, "last_confirmed", &last_confirmed);
        if (err != TIBRV_OK)
        {
        ...
        {
            printf("      %-45s Last confirmed:  %10llu\n", listener_cmname, last_confirmed);
        }
    }
     ...
     */
    printf("                    %llu messages occupy %llu bytes.\n", total_msgs, total_size);
...
main(int argc, char **argv)
{
     ...
     */
    get_InitParms(argc, argv, MIN_PARMS, &serviceStr,
