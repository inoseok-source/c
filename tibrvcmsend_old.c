    ...
    tibrv_f64* cmtimeNum)
{
    ...
    while ( i+2 <= argc && *argv[i] == '-' )
    {
        ...
        else if (strcmp(argv[i], "-cmtime") == 0) // CMTIME 옵션 처리 로직 추가
        {
    ...
    void *     closure)
{
    ...
    {
        tibrvEvent_Destroy(event);
        // 프로그램 종료를 위해 여기서 tibrv_Close()를 호출하거나,
        // 메인 루프가 종료될 수 있도록 별도의 플래그를 설정할 수 있습니다.
        // 현재 로직은 tibrvQueue_Dispatch가 TIBRV_OK를 반환하지 않으면 종료됩니다.
        return; 
    }
...
main(int argc, char **argv)
{
    ...
    //DELIVERY.FAILED(sender), DELIVERY.UNAVAILABLE(listener) 확인, 
    //sender ledger에서 메시지를 삭제하는 만료시간
    err = tibrvcmTransport_SetDefaultCMTimeLimit(cmtransport, cmtimeNum);

    if (err != TIBRV_OK)
    {
        fprintf(stderr, "%s: Failed to set CM time limit --%s\n",
                progname, tibrvStatus_GetText(err));
        exit(1);
    }
